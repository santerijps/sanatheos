...
│   ├── styleguide.json       # Paragraph/poetry formatting data (copied)
...
3. **Loads styleguide** — fetches `styleguide.json` and passes it to `setStyleguide()` in render.ts for paragraph/poetry formatting.
...
7. **Wires up all event listeners:**
...
| `renderStyledVerses` | Renders verses with paragraph breaks, poetry indentation, stanza breaks |
...
3. Copies static files: `index.html`, `style.css`, `robots.txt`, `favicon.ico`, `manifest.json`, `sw.js`, `pwaicon-192.png`, `pwaicon-512.png`, `styleguide.json`.
...
The styleguide is fetched once on app startup (`fetch("./styleguide.json")`) and stored via `setStyleguide()` in `render.ts`. The `renderStyledVerses()` helper applies formatting to all verse-rendering functions.
...
285 tests across 3 files using `bun test`.
